...
// <summary>Textベースクラス</summary>
    ...
    public abstract class BaseText : UIBehaviour
    {
        ...
        [SerializeField]
        private ColorEntryId _colorEntryId = new();

        /// <summary>フォントのサイズ</summary>
        [SerializeField]
        private FontSize _fontSize = FontSize.Px24;

        /// <summary>テキストが領域から出た時の種別</summary>
        [SerializeField]
        private TextOverflowType _textOverflowType = TextOverflowType.Shrink;
        ...
        public void SetText(string text, bool updateFontSize = true)
        {
        ...
        public void SetRichText(bool richText, bool raycastControl = false)
        {
            ...
            if (raycastControl)
            {
                _textMeshPro.raycastTarget = richText;
            }
        }
        ...
        {
            PaletteStore.Instance.ColorPalette.TryGetActiveValue(_colorEntryId.Value, out var value);

            if (value != null)
            {
                Observable.EveryValueChanged(value, x => x.Value)
                    .Subscribe(color => _textMeshPro.color = color).AddTo(this);
            }
        }
